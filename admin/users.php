...
<body>
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
            	<table class="table table-bordered table-striped table-hover" id="activities_table">
        			...
        			<tbody id="tbody_activites_table">
        				<?php
        					...
        					<tr>
        						...
        						<td>
        							<?php if($value['status'] == '1') { ?>
        								<button class="btn btn-success btn-md btn_active_status" value="1" data-id="<?php echo $value['user_id']?>">Active</button>
        							<?php } else { ?> 
        								<button class="btn btn-danger btn-md btn_active_status" value="0" data-id="<?php echo $value['user_id']?>">InActive</button>
        							<?php } ?>
                                    &nbsp;
                                    <button type="button" class="btn btn-danger delete_user" title="Delete User" data-id="<?= $value['user_id'] ?>"><i class="fa fa-trash"></i></button>	
        						</td>
        					</tr>
        				<?php		
        					}
    ...
    <script type="text/javascript">
        $(document).ready(function() {
           	...
           	$(document).on('click','.btn_active_status',function(){
           		var status = $(this).val();
                ...
                if (confirm("Do you want to delete User?")) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            var activies_tbody = '';
                            $("#tbody_activites_table").html('');
                            $.each(res, function(index, value){
                                /*console.log(value);*/
                                activies_tbody += '<tr>';
                                activies_tbody += '<td>'+value.firstname+'</td>';
                                activies_tbody += '<td>'+value.lastname+'</td>';
                                activies_tbody += '<td>'+value.email+'</td>';
                                activies_tbody += '<td>'+value.mobile+'</td>';
                                activies_tbody += '<td>'+value.city+'</td>';
                                activies_tbody += '<td>'+value.dob+'</td>';
                                activies_tbody += '<td>'+value.gender+'</td>';
                                activies_tbody +='<td>';
                                if(value.status == '1'){
                                    activies_tbody +="<button class='btn btn-success btn-md btn_active_status' value='1' data-id="+value.user_id+">Active</button>";
                                } else {
                                    activies_tbody +="<button class='btn btn-danger btn-md btn_active_status' value='0' data-id="+value.user_id+">InActive</button>";
                                }
                                activies_tbody +="&nbsp;";
                                activies_tbody +="<button type='button' class='btn btn-danger delete_user' title='Delete User' data-id="+value.user_id+"><i class='fa fa-trash'></i></button>";
                                activies_tbody +='</td>';
                                activies_tbody += '</tr>';
                            });
                            $("#tbody_activites_table").html(activies_tbody);
                        }
                    });
                }
            });
